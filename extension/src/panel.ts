...
export interface Report {
  ...
  avoidable: boolean;
  memoized?: boolean;
  /** Compiled by React Compiler (output memoized per input). */
  compiled?: boolean;
...
function createPanel(root: HTMLElement, transport: Transport, options: PanelOptions = {}): Panel {
  ...
  /** A prefilled GitHub issue: everything a bug report needs and nothing from the page's data. */
  function issueUrl(): string {
    ...
    const params = new URLSearchParams({ title: 'Panel: ', body: lines.join('\n'), labels: 'bug' });
    return `https://github.com/NexaLeaf/rerender-lens/issues/new?${params.toString()}`;
  }
