...
- `test/vite.test.ts` aliases `rerender-lens` to `src/index.ts`: the example links the package, whose exports point at `dist/`, and `npm run check` runs tests before the build.
